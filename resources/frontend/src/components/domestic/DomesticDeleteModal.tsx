...
interface DomesticDeleteModalProps {
  deletingTx: any
  onClose: () => void
  onConfirm: () => void
  saving: boolean
...
export default function DomesticDeleteModal({
  deletingTx,
  onClose,
  onConfirm,
  saving,
}: DomesticDeleteModalProps) {
  const { tokens } = useApp()
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 13, color: tokens.text, lineHeight: 1.45 }}>
          Apakah Anda yakin ingin menghapus data transaksi <strong>{deletingTx.id}</strong> (Tanggal: {deletingTx.date} - Total: {Number(deletingTx.totalKg ?? deletingTx.total_weight_kg ?? 0).toFixed(1)} kg)?
        </div>
        ...
        >
          <div>👤 Diinput oleh: <strong style={{ color: tokens.text }}>{deletingTx.creator?.name || deletingTx.picName || 'Petugas EHS'}</strong> ({deletingTx.date})</div>
          {deletingTx.session && (
            <div>⏰ Sesi: {deletingTx.session === 'MORNING' || deletingTx.session === 'morning' ? 'Pagi' : 'Sore'}</div>
          )}
        </div>
