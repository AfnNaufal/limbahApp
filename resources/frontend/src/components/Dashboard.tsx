    ...
    <div style={{ marginBottom: 28 }}>
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns:
            'minmax(190px, 220px) minmax(250px, 1fr) minmax(180px, 200px) minmax(250px, 1fr)',
          gap: 12,
          overflowX: 'auto',
        }}
      >
...
function QuickPreview({
  ...
  const labels: Record<string, string> = {
    b3in: 'B3 Masuk',
    b3out: 'B3 Keluar',
    domMorning: 'Domestik Organik',
    domAfternoon: 'Domestik Anorganik',
  }
  ...
  const stats =
    statsById[id as keyof typeof statsById] ??
    SUMMARY_STATS.b3In
...
export default function Dashboard() {
  ...
  const b3InWeight =
    Number(summaryData?.b3_in_weight_kg) ||
    SUMMARY_STATS.b3In.total

  const b3OutWeight =
    Number(summaryData?.b3_out_weight_kg) ||
    SUMMARY_STATS.b3Out.total

  const domesticOrganicWeight =
    Number(
      summaryData?.domestic_today_organic_kg,
    ) ||
    SUMMARY_STATS.domMorning.total

  const domesticInorganicWeight =
    Number(
      summaryData?.domestic_today_inorganic_kg,
    ) ||
    SUMMARY_STATS.domAfternoon.total
  ...
  const categories: CategoryConfig[] = [
    {
      ...
      barData: MONTHLY_B3_IN.map((item) => ({
        name: item.month,
        value: item.value,
      })),
      weeklyData: WEEKLY_B3_IN.map((item) => ({
        name: item.week,
        value: item.value,
      })),
      monthlyData: MONTHLY_B3_IN.map((item) => ({
        name: item.month,
        value: item.value,
      })),
      pieData: PIE_B3_IN,
      stats: {
        ...
        entries:
          summaryData?.b3_count_in ??
          SUMMARY_STATS.b3In.entries,
      },
      unit: 'kg',
    },
    {
      ...
      barData: MONTHLY_B3_OUT.map((item) => ({
        name: item.month,
        value: item.value,
      })),
      weeklyData: WEEKLY_B3_OUT.map((item) => ({
        name: item.week,
        value: item.value,
      })),
      monthlyData: MONTHLY_B3_OUT.map((item) => ({
        name: item.month,
        value: item.value,
      })),
      pieData: PIE_B3_OUT,
      stats: {
        ...
        entries:
          summaryData?.b3_count_out ??
          SUMMARY_STATS.b3Out.entries,
      },
      unit: 'kg',
    },
    {
      ...
      barData: MONTHLY_DOM_MORNING.map(
        (item) => ({
          name: item.month,
          value: item.value,
        }),
      ),
      weeklyData: WEEKLY_DOM_MORNING.map(
        (item) => ({
          name: item.week,
          value: item.value,
        }),
      ),
      monthlyData: MONTHLY_DOM_MORNING.map(
        (item) => ({
          name: item.month,
          value: item.value,
        }),
      ...
      stats: {
        ...
        entries:
          SUMMARY_STATS.domMorning.entries,
      },
      unit: 'kg',
    },
    {
      ...
      barData: MONTHLY_DOM_AFTERNOON.map(
        (item) => ({
          name: item.month,
          value: item.value,
        }),
      ),
      weeklyData: WEEKLY_DOM_AFTERNOON.map(
        (item) => ({
          name: item.week,
          value: item.value,
        }),
      ),
      monthlyData: MONTHLY_DOM_AFTERNOON.map(
        (item) => ({
          name: item.month,
          value: item.value,
        }),
      ...
      stats: {
        ...
        change:
          SUMMARY_STATS.domAfternoon.change,
        entries:
          SUMMARY_STATS.domAfternoon.entries,
      },
      unit: 'kg',
    },
  ]
  ...
  const connectionText = loading
    ...
    : apiError
      ...
      : 'Terhubung ke Database Live (MySQL)'
    ...
    >
      ...
      >
        {[
          ...
          {
            id: 'domMorning',
            label: 'Domestik Organik',
            value: domesticOrganicWeight,
            change:
              SUMMARY_STATS.domMorning.change,
            color: tokens.chartDomMorning,
          },
          {
            id: 'domAfternoon',
            label: 'Domestik Anorganik',
            value: domesticInorganicWeight,
            change:
              SUMMARY_STATS.domAfternoon.change,
            color: tokens.chartDomAfternoon,
          },
